...
| **Phase 4** | Punctuation | OfflinePunctuation unterstützt optional segmentierte Text-Orchestration mit Caller-owned `OfflineTextBuffer` out; neue `StreamingPunctuationEngine` nutzt echtes `OnlinePunctuation` über `LiveTextBuffer` in/out; `TextPunctuationAssistedEvaluator` löst Offline- und Online-Punctuation-Instanzen via `punctuationInstanceId` auf. **Status: Completed** (Jest: `punctuation-segmented`, `streaming-punctuation`, `segmentation-engine-punctuation`; Android Kotlin: Example `:react-native-sherpa-onnx:compileDebugKotlin`; iOS Example `xcodebuild` Debug simulator). |
| **Phase 5** | TTS (Incremental entfernen, + `tts_produced` Links) | Vier Modi implementiert (Offline off/auto, Streaming off/auto), segmentierter Offline-Orchestrator + `tts_produced` LinkMap aktiv, Incremental-Legacy entfernt. **Status: Completed** (Jest: `synthesize-mode1-oneshot`, `synthesize-mode2-segmented`, `synthesize-mode2-linkmap`, `streaming-mode3`, `streaming-mode4-segmentation`, `orchestrate`, `offline-orchestrator`). |
| **Phase 6** | Alignment (Engine-first, + `alignment` Links) | `AlignmentEngine`-basierter segmented-accurate Pfad (`asr_mediated`/`chunked_forced_ctc`) integriert, `SegmentLinkMap`-Nutzung aktiv, keine Legacy-Fake-Live-Reaktivierung. **Status: Completed** (Jest: `src/alignment`, `segment-link-map`; Contract: `linkType: 'alignment'`, no-silent-fallback). |
| **Phase 7** | Cleanup & Contract-Parity (Sub-Plan 06) | Systematischer Soll-Ist-Audit aller Sub-Pläne, verbleibende Abweichungen nachziehen (inkl. **JSI-Fast-Path Kandidaten** mit Priorisierung/Messung), Legacy-/Dead-Code entfernen, release-ready Hardening. **Explizit enthalten:** cross-feature Harmonisierung von `segmentation.mode` (`off`/`manual`/`auto`) inkl. einheitlicher Fehlersemantik (`SEGMENTATION_POLICY_INVALID`) und Mindest-Testmatrix pro Feature (STT, TTS, Punctuation, Enhancement, Alignment). |
