/**
 * Audio Tagging model discovery + path resolution.
 */
...
const RECOMMENDED_AUDIO_TAGGING_MODEL_IDS =
  RECOMMENDED_MODEL_IDS[ModelCategory.AudioTagging] ?? [];

export type AudioTaggingModelEntry = {
  id: string;
  label: string;
  recommended?: boolean;
};

export type AudioTaggingCatalogSnapshot = {
  entries: AudioTaggingModelEntry[];
  padModelIds: string[];
...
export function isAudioTaggingModelFolder(
  folder: string,
  hint: string
): boolean {
  if (
  ...
  return (
    normalized.includes('audio-tagging') ||
    normalized.includes('audio_tagging') ||
    normalized.includes('audiotagging') ||
    (normalized.includes('ced') && normalized.includes('tagging')) ||
    (normalized.includes('zipformer') && normalized.includes('tagging'))
  );
}
...
): AudioTaggingModelEntry[] {
  const uniqueEntries = Array.from(
    new Map(entries.map((entry) => [entry.id, entry])).values()
  );
  const recommendedSet = new Set(recommendedIds);
  const recommended: AudioTaggingModelEntry[] = [];
  const remaining: AudioTaggingModelEntry[] = [];
...
export function getAudioTaggingModelPathConfig(
  modelId: string,
  ctx: {
    padModelIds: string[];
    padModelsPath: string | null;
  ...
  if (ctx.padModelIds.includes(modelId)) {
    return ctx.padModelsPath
      ? getFileModelPath(modelId, ModelCategory.AudioTagging, ctx.padModelsPath)
      : getFileModelPath(modelId, ModelCategory.AudioTagging);
  }
  if (ctx.downloadedIds.has(modelId)) {
    return getFileModelPath(modelId, ModelCategory.AudioTagging);
  }
...
export async function loadAudioTaggingModelCatalog(): Promise<AudioTaggingCatalogSnapshot> {
  const assetModels = await listAssetModels();
  const bundledIds = assetModels
    .filter((model) => isAudioTaggingModelFolder(model.folder, model.hint))
  ...
  const downloaded = await listDownloadedModels(ModelCategory.AudioTagging);
  ...
  const entries = prioritizeEntries(
    ...
    RECOMMENDED_AUDIO_TAGGING_MODEL_IDS
  );
