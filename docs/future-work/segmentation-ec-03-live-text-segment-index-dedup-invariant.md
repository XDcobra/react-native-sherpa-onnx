...
In `src/textbuffer/index.ts`, `dispatchLiveTextSegmentEvent` **drops** native `pipelineLiveTextSegment` deliveries when `segment.segmentIndex <= lastSegmentIndex` (monotonic deduplication keyed by buffer).
...
1. **Document or enforce an invariant:** For a given live text buffer, `segmentIndex` in `pipelineLiveTextSegment` is **strictly increasing** per commit order (including STT / worker paths). Add the invariant to `sub-03-buffer-integration.md` or live-text buffer public docs once verified.
...
3. **Verification:** Short audit of all `pipelineLiveTextSegment` emitters (Android / iOS / workers) and at least one test per critical path: “monotonic `segmentIndex`”.
...
- [ ] Audit: all `pipelineLiveTextSegment` emitters (Android / iOS / STT worker).
