...
| --- | --- | --- | --- | --- |
| **`accurate`** + `segmentation.mode === 'auto'` + **`chunked_forced_ctc`** | `runAccurateChunkedForcedCtc` | **TBD impl:** derive from driver plan (e.g. anchor windows / forced-CTC chunks — exact definition in implementation PR). | `0 .. totalSegments-1` in driver order | Emit **one** event: `currentSegment=0`, `totalSegments=1`, `fraction=0` at start; optional second tick at end is **out of scope** (orchestrator does not do end-tick). |
| **`accurate`** + `segmentation.mode === 'auto'` + **`asr_mediated`** | `runAccurateAsrMediated` | **TBD impl:** ASR-mediated passes (anchor + hypothesis alignment passes). | Same as left | Same single-shot fallback |
