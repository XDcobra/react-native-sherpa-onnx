static std::string convertF32leMonoFileToFormat(const char* rawPath, int pcmSampleRate, const char* outputPath, const char* formatHint, int outputSampleRateHz) {
#ifdef HAVE_FFMPEG
    ...
    struct stat stIn = {};
    long inputSizeBytes = (stat(rawPath, &stIn) == 0 && S_ISREG(stIn.st_mode)) ? (long)stIn.st_size : -1;
    LOGI("convertF32leMono: rawPath=%s inputSizeBytes=%ld format=%s outputPath=%s sr=%d", rawPath ? rawPath : "(null)", inputSizeBytes, formatHint ? formatHint : "", outputPath ? outputPath : "(null)", pcmSampleRate);

    AVFormatContext* inFmt = nullptr;
    const AVInputFormat* ifmt = av_find_input_format("f32le");
    if (!ifmt) {
        return std::string("f32le demuxer not available in this FFmpeg build");
    }
    AVDictionary* rawOpts = nullptr;
    char srbuf[32];
    snprintf(srbuf, sizeof(srbuf), "%d", pcmSampleRate);
    av_dict_set(&rawOpts, "sample_rate", srbuf, 0);
    av_dict_set(&rawOpts, "channels", "1", 0);
    if (avformat_open_input(&inFmt, rawPath, ifmt, &rawOpts) < 0) {
        av_dict_free(&rawOpts);
        LOGE("Failed to open raw f32le input: %s", rawPath ? rawPath : "(null)");
        return std::string("Failed to open raw PCM input");
    }
    av_dict_free(&rawOpts);
    if (avformat_find_stream_info(inFmt, nullptr) < 0) {
        avformat_close_input(&inFmt);
        return std::string("Failed to find stream info for raw PCM");
    }
    ...
    return std::string("");
#else
    (void)rawPath; (void)pcmSampleRate; (void)outputPath; (void)formatHint;
