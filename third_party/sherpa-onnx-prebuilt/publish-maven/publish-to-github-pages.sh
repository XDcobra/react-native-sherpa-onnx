...
# Source env file if present (provides GROUP_ID, ARTIFACT_ID, optional DEPENDENCY_*)
...
if [ -n "${DEPENDENCY_GROUP_ID}" ] && [ -n "${DEPENDENCY_ARTIFACT_ID}" ] && [ -n "${DEPENDENCY_VERSION}" ]; then
  python3 -c "
import sys
g, a, v, dg, da, dv, path = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7]
dep = f'''  <dependencies>
    <dependency>
      <groupId>{dg}</groupId>
...
<project xmlns=\"http://maven.apache.org/POM/4.0.0\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://maven.apache.org/POM/4.0.0 https://maven.apache.org/xsd/maven-4.0.0.xsd\">
  ...
  <name>{a}</name>
  <description>Sherpa-onnx for Android (QNN). Built from react-native-sherpa-onnx. Depends on com.xdcobra.sherpa:onnxruntime.</description>
{dep}
</project>'''
open(path, 'w').write(pom)
" "$GROUP_ID" "$ARTIFACT_ID" "$MAVEN_VERSION" "$DEPENDENCY_GROUP_ID" "$DEPENDENCY_ARTIFACT_ID" "$DEPENDENCY_VERSION" "$VERSION_PATH/${POM_NAME}"
else
  python3 -c "
import sys
...
<project xmlns=\"http://maven.apache.org/POM/4.0.0\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://maven.apache.org/POM/4.0.0 https://maven.apache.org/xsd/maven-4.0.0.xsd\">
  ...
  <packaging>aar</packaging>
  <name>''' + a + '''</name>
  <description>Sherpa-onnx for Android (QNN). Built from react-native-sherpa-onnx.</description>
</project>''')
