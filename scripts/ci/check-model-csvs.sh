...
# kws-models, audio-tagging-models)
...
KWS_CSV="${KWS_CSV:-test/fixtures/kws-models-expected.csv}"
AUDIO_TAGGING_CSV="${AUDIO_TAGGING_CSV:-test/fixtures/audio-tagging-models-expected.csv}"
...
if [ ! -f "$KWS_CSV" ]; then
  echo "::warning::Missing $KWS_CSV (run from repo root or set KWS_CSV)"
  exit 0
fi
if [ ! -f "$AUDIO_TAGGING_CSV" ]; then
  echo "::warning::Missing $AUDIO_TAGGING_CSV (run from repo root or set AUDIO_TAGGING_CSV)"
  exit 0
fi
...
# Fetch kws-models release assets (.tar.bz2)
KWS_ASSETS=""
KWS_RESP="${KWS_RESP:-$(curl "${CURL_GH_API[@]}" "https://api.github.com/repos/${REPO}/releases/tags/kws-models")}"
if echo "$KWS_RESP" | jq -e '.assets' >/dev/null 2>&1; then
  KWS_ASSETS=$(echo "$KWS_RESP" | jq -r '.assets[] | select(.name | endswith(".tar.bz2") or endswith(".onnx")) | .name')
else
  echo "::warning::Could not fetch kws-models release or it has no assets"
fi

# Fetch audio-tagging-models release assets (.tar.bz2)
AUDIO_TAGGING_ASSETS=""
AUDIO_TAGGING_RESP="${AUDIO_TAGGING_RESP:-$(curl "${CURL_GH_API[@]}" "https://api.github.com/repos/${REPO}/releases/tags/audio-tagging-models")}"
if echo "$AUDIO_TAGGING_RESP" | jq -e '.assets' >/dev/null 2>&1; then
  AUDIO_TAGGING_ASSETS=$(echo "$AUDIO_TAGGING_RESP" | jq -r '.assets[] | select(.name | endswith(".tar.bz2") or endswith(".onnx")) | .name')
else
  echo "::warning::Could not fetch audio-tagging-models release or it has no assets"
fi
...
KWS_CSV_NAMES=$(csv_asset_names "$KWS_CSV")
AUDIO_TAGGING_CSV_NAMES=$(csv_asset_names "$AUDIO_TAGGING_CSV")
...
while IFS= read -r asset; do
  [ -z "$asset" ] && continue
  if ! grep -qFx -- "$asset" <<< "$KWS_CSV_NAMES"; then
    KWS_MISSING="${KWS_MISSING}  - ${asset}\n"
  fi
done <<< "$KWS_ASSETS"

AUDIO_TAGGING_MISSING=""
while IFS= read -r asset; do
  [ -z "$asset" ] && continue
  if ! grep -qFx -- "$asset" <<< "$AUDIO_TAGGING_CSV_NAMES"; then
    AUDIO_TAGGING_MISSING="${AUDIO_TAGGING_MISSING}  - ${asset}\n"
  fi
done <<< "$AUDIO_TAGGING_ASSETS"

if [ -n "$ASR_MISSING" ] || [ -n "$TTS_MISSING" ] || [ -n "$SPEECH_MISSING" ] || [ -n "$VAD_MISSING" ] || [ -n "$SEPARATION_MISSING" ] || [ -n "$SPEAKER_EMB_MISSING" ] || [ -n "$DIARIZATION_MISSING" ] || [ -n "$KWS_MISSING" ] || [ -n "$AUDIO_TAGGING_MISSING" ]; then
  ...
  [ -n "$KWS_MISSING" ] && echo -e "KWS (kws-models) assets missing from $KWS_CSV:\n$KWS_MISSING"
  [ -n "$AUDIO_TAGGING_MISSING" ] && echo -e "Audio tagging (audio-tagging-models) assets missing from $AUDIO_TAGGING_CSV:\n$AUDIO_TAGGING_MISSING"
  echo "Please run the collect workflows to update fixtures:"
  echo "  - Testdata - Collect model structures (workflow_dispatch; stream=all, asr, tts, punctuation, enhancement, separation, speaker-embedding, diarization, kws, or audio-tagging)"
  exit 0
fi
