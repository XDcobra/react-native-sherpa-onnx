...
export type LanguageIdentificationCustomInitFormState = {
  modelType: LanguageIdModelType;
  fileSources: Partial<Record<LanguageIdCustomPathKey, FileSource>>;
};

type LanguageIdentificationCustomInitFormProps = {
  value: LanguageIdentificationCustomInitFormState;
  onChange: (next: LanguageIdentificationCustomInitFormState) => void;
...
const PATH_LABELS: Record<LanguageIdCustomPathKey, string> = {
  encoder: 'Encoder ONNX',
  decoder: 'Decoder ONNX',
};

export function LanguageIdentificationCustomInitForm({
  value,
  onChange,
  ...
  disabled = false,
  fillHint = null,
}: LanguageIdentificationCustomInitFormProps) {
  ...
  useEffect(() => {
    ...
    getCustomModelPathRequirements(ModelCategory.LanguageId, value.modelType)
      .then((requirements) => {
        if (!cancelled) {
  ...
  const slotKeys = useMemo(
    () =>
      schema.fields.map((field) => ({
        key: field.key as LanguageIdCustomPathKey,
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.subheading}>Language ID model type</Text>
      <ScrollView
        horizontal
      ...
      <Text style={styles.hint}>
        Multilingual Whisper encoder + decoder (same pack as STT Whisper). Use
        Fill to pre-populate from the catalog selection.
      </Text>
