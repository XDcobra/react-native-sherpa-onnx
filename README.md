...
| **Zipvoice**     | `'zipvoice'`      | Standard TTS with **`sid`**. **Voice cloning** via **`voiceClone`** on batch `synthesize` (live Zipvoice clone not supported). **`numSteps`** (flow-matching) applies with cloning; omit for upstream default (**5**). Cloning is **supported on Android & iOS**. Encoder + decoder + vocoder. | [Download](https://k2-fsa.github.io/sherpa/onnx/tts/pretrained_models/zipvoice.html) |
| **Pocket**       | `'pocket'`        | Flow-matching TTS. **Voice cloning** on **Android:** batch and live. **iOS:** cloning is experimental. **`numSteps`** applies with `voiceClone`. Detected by lm_flow, lm_main, text_conditioner, vocab/token_scores. | [Download](https://github.com/k2-fsa/sherpa-onnx/releases/tag/tts-models) |
| **Supertonic**    | `'supertonic'`        | Fast on-device TTS. **`numSteps`** (diffusion) works **without** `voiceClone` on Android & iOS (batch and live). | [Download](https://github.com/k2-fsa/sherpa-onnx/releases/tag/tts-models) |
