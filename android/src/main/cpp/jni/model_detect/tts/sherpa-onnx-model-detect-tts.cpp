...
static void FillTtsDerivedCatalogMetadata(TtsDetectResult& r, const std::string& idForHeuristics) {
...
TtsDetectResult DetectTtsModel(
    ...
    if (!has_dir && !has_asset) {
        result.error = "TTS: modelDir and assetName are both empty";
    ...
    if (has_asset) {
        ...
        FillTtsDerivedCatalogMetadata(result, BasenameLastPathComponent(modelDir));
    }
