...
Implement the wrapper with `cause: err` so debugging retains the original failure chain.

```ts
throw new LiveOfflinePipelineError(featureName, message, { cause: err });
```

`LiveOfflinePipelineError` constructor should accept `ErrorOptions` and forward to `super(message, options)`.

### OQ-1.2 — Should the validator return `{ policy, mode }` or just `{ policy }`?

**Decision: Return `{ policy }` only (accepted).**

The live overload is always segmentation-driven (`mode: 'auto'`), so returning `mode` adds no value and can create ambiguity.

### OQ-1.3 — Add `LIVE_OFFLINE_POLICY_NOT_SUPPORTED` as a separate code for enhancement-style restrictions?

**Decision: No additional code. Keep one code only (accepted).**

Continue using only:

- `LIVE_OFFLINE_SEGMENTATION_REQUIRED`

for missing/invalid/unsupported live-offline segmentation inputs, including evaluator restrictions (for example enhancement requiring `continuous_frames`). Detailed remediation stays in the message text.
