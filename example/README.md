...
**Codec sandbox (`test_codec/`):** bundled samples for probe/decode/encode round-trips. Add files listed in `example/android/app/src/main/assets/test_codec/README.md` (Android) and `example/ios/sherpa_models/test_codec/README.md` (iOS), then rebuild. Runtime path: `test_codec/sample.<ext>` via `{ kind: 'app', base: 'files', path: '…' }`.
